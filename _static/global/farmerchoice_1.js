...
let farmerchoices = new function() {
...
let other_bin_tokens = function(bin) {
	let count = 0;
	for (i = 1; i <= farmerchoices.beldat["num_bins"] ; i++) {
		if (i == bin) continue
...
let fix_slider_position = function() {
	...
	let num_bins = farmerchoices.beldat["num_bins"];
...
this.mkview = function(belief_farms, bel_div, results = 0, show_submit = 1) {
	// Make this global to the namespace
	farmerchoices.belief_farms = belief_farms;

	farmerchoices.beldat = belief_farms["beldat"];

	let hex_colors = belief_farms["hex_colors"];
	let beldat = belief_farms["beldat"];
	let tokens = belief_farms["tokens"];
	let currency = belief_farms["currency"];
	...
	// Fill in some elements from belief_farms
	$("#bar_div").css("border", "1px solid " + hex_colors[0]);
	$(".div_bar").css("background-color", hex_colors[0]);
	$("#round_number").html(belief_farms["round_number"]);
	$("#number_of_rounds").html(belief_farms["number_of_rounds"]);

	if (results == 1) {
		$("#pay_round").html(belief_farms["pay_round"]);
	}
	...
	for (let i = 1 ; i <= num_bins; i++ ) {
		...
		if (results == 1) {
			$("#bin" + i).val(belief_farms["pay_choices"][i - 1]);
		}

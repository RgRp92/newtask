...
let farmerschoice = new function() {
...
let other_bin_tokens = function(bin) {
	let count = 0;
	for (i = 1; i <= farmerschoice.beldat["num_bins"] ; i++) {
		if (i == bin) continue
...
let fix_slider_position = function() {
	...
	let num_bins = farmerschoice.beldat["num_bins"];
...
this.submit_click = function () {log_event('submit');}
...
let beliefs_add_content = function(num_bins, tokens, divid, results = 0, show_submit = 1) {
	...
	inhtml += "</div>\n";
	if (farmerchoices.beldat["bin_button"] != "") {
	}
	inhtml += "</td>\n";
...
this.mkview = function(belief_farmers, bel_div, results = 0, show_submit = 1) {
	// Make this global to the namespace
	farmerschoice.belief_farmers = belief_farmers;

	farmerschoice.beldat = belief_farmers["beldat"];

	let hex_colors = belief_farmers["hex_colors"];
	let beldat = belief_farmers["beldat"];
	let tokens = belief_farmers["tokens"];
	let currency = belief_farmers["currency"];
	...
	// Fill in some elements from belief_farmers
	$("#bar_div").css("border", "1px solid " + hex_colors[0]);
	$(".div_bar").css("background-color", hex_colors[0]);
	$("#round_number").html(belief_farmers["round_number"]);
	$("#number_of_rounds").html(belief_farmers["number_of_rounds"]);

	if (results == 1) {
		$("#pay_round").html(belief_farmers["pay_round"]);
	}
	...
	for (let i = 1 ; i <= num_bins; i++ ) {
		...
		if (results == 1) {
			$("#bin" + i).val(belief_farmers["pay_choices"][i - 1]);
		}
